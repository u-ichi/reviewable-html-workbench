...
description: |
  HTMLを最終成果物として生成・検証・プレビューしたい時に使う共通レンダラー。現行rendererの表現能力を前提にagentが文書モデルを直接設計し、表・リスト・コード・注記・図・生成画像を選んだHTML bundleとセッション限定のプレビューURLを提示する。Triggers: html出力して, HTMLにして, HTMLで出して, この内容をHTMLで出して, HTMLでプレビューして, HTMLレンダラー, HTML出力を共通化, 図示つきHTML, visual HTML renderer。使用しない場面: 設計内容そのものの作成、Notion投稿だけ、既存HTMLの軽微な見た目修正だけ。
...
| `3` | `<h3>` | 節見出し。章の中を細分化するサブセクション（省略時のデフォルト） |
| — | `<h4>` | `html` block の content 内で使う小見出し。block の `heading_level` ではなく content の HTML で直接書く |

- 4ブロック以上の文書では、少なくとも1つの `heading_level: 2` ブロックを置く。
- `heading_level: 3` のブロックは、先行する `heading_level: 2` ブロックの配下に位置づける。
- 3ブロック以下のフラットな文書では `heading_level` を省略してよい。
- `html` block 内で小見出しが必要な場合は `<h4>` を使う。rendererが content 内の `<h3>` を `<h4>` へ自動シフトするため、content 内に `<h2>` や `<h3>` を直接書かない。
...
- 読者と用途。
- 章構成。文書全体を概要→各論→結論のような読み手を誘導する流れに分ける。4ブロック以上の文書では、どのブロックが章（`heading_level: 2`）でどのブロックが節（`heading_level: 3`）かを先に決める。1つの章に節が偏りすぎないようバランスを取る。
- 主要な情報単位。
...
- 4ブロック以上の文書で `heading_level: 2` のブロックが少なくとも1つある。`heading_level: 3` のブロックが最初の `heading_level: 2` ブロックより前に出現していない。
