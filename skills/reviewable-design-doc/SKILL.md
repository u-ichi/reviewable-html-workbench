...
7. `render` CLIでHTML bundleを生成する。
8. `validate` CLIでHTML bundleを検証する。
9. ユーザー向け最終HTMLでは既定で `preview` CLIを `--mode auto` で起動し、返却JSONの `url` と `stop_command` を最終応答に必ず書く。
10. レビュー完了後は `ingest-review` CLIでコメントを分類し、必要に応じて設計へ反映する。
11. 確認が必要なコメントには、チャットだけでなくHTMLコメントへagent replyを書き戻す。
...
- 比較・代替案・評価軸は表、処理・依存・構成はdiagramブロック、決定・前提・注意はcallout、操作例・ログ・コマンドはコードブロック、レビューしてほしい論点は専用のレビュー観点blockにする。
