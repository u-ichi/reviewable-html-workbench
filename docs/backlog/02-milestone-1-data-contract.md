...
- [ ] `document-model.json` schemaが定義されている。
  - 検証: `test -f schemas/document-model.schema.json`
- [ ] `comments.json` schemaが定義されている。
  - 検証: `test -f schemas/comments.schema.json`
- [ ] `preview-session.json` schemaが定義されている。
  - 検証: `test -f schemas/preview-session.schema.json`
- [ ] 最小fixtureとschema validation testがある。
  - 検証: `python3 -m unittest discover -s tests`
- [ ] schema変更が `docs/development-plan.html` と `docs/design.html` に反映されている。
  - 検証: `rg -n "document-model|comments.json|preview-session" docs/development-plan.html docs/design.html`

## やること

- [ ] `schemas/` を作成する。
- [ ] `tests/fixtures/minimal_document_model.json` を追加する。
- [ ] schema validation helperを実装する。
- [ ] コメントreply/statusのschemaを明確化する。
